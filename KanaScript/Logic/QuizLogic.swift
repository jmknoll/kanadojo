...
/// - Parameter score: For typeB, the embedding quality score (0–1) from KanaEmbeddingScorer.
...
// MARK: - Spaced Repetition (SM-2) — Word variant

/// Apply SM-2 to a WordProgress record (single quiz type, no quality score).
func applyWordSpacedRepetition(to progress: WordProgress, correct: Bool) {
    if correct {
...
// MARK: - Weighted Sample — Word variant

func wordWeightedSample(
    _ pool: [WordEntry],
    count: Int,
    ...
    while result.count < n && !remaining.isEmpty {
        let weights = remaining.map { entry -> Double in
            let strength = progressDict[entry.id]?.strength ?? 0
